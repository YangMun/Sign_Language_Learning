...
window.geometry('650x800+5+5')  # 전체화면으로 설정
window.title("모두를 위한 수화 학습")

window.configure(background='gray') # 메인 화면 배경색 설정
...
style.set_theme("equilux")  # 원하는 테마 설정
...
def study_page():
    ...
    word_window.geometry('650x800+5+5')
    word_window.configure(bg="gray")  # 배경색 설정

    font = tkfont.Font(family="Pretendard-Bold", size=30, slant="roman", weight="bold") # 상단의 라벨 적기

    label = tk.Label(word_window,text = 'Study!', font=font,bg='gray') # top 쪽에 올라감
    label.place(x=265,y=35)

    # 왼쪽 프레임 만들기 (동영상 추출 공간) (650 * 628)    
    l_frame = tk.Frame(word_window, width= 650, height=628, bg='gray') # 왼쪽프레임
    l_frame.pack(side=tk.LEFT)

    # 프레임 위에 동영상 넣기
    canvas = tk.Canvas(l_frame, width=650, height=600, bg='gray')
...
def quiz_page():
    ...
    quiz_window.geometry("650x500+5+5")  # 창크기
    quiz_window.resizable(width=False, height=False) # 창크기 조절 비활성
    quiz_window.configure(bg="gray")  # 배경색 설정
    ...
    label.pack()
    label.configure(bg="gray")  # 배경색 설정

    # 영상 프레임 표시를 위한 캔버스 생성
    canvas = tk.Canvas(quiz_window, width=620, height=400)
    canvas.pack()
    ...
    result_label.configure(bg="gray")  # 배경색 설정
    ...
    action_label.configure(bg="gray")  # 배경색 설정
    ...
    skip_button.place(x=578, y=5)  # 좌표 지정하여 배치
...
style = ttk.Style()
style.configure("Custom.TButton", background="gray", foreground="white",font=("Pretendard-Bold", 20), padding=45)

# 학습 버튼 테마 적용
button1 = ttk.Button(window, text="학습",style="Custom.TButton", command=study_page) #퀴즈 버튼 클릭 시 study_page() 함수 호출
button1.grid(row=1, column=0, sticky="nsew")  # 첫 번째 버튼을 두 번째 행(row)의 첫 번째 열(column)에 배치

# 퀴즈 버튼 테마 적용
button2 = ttk.Button(window, text="퀴즈", style="Custom.TButton", command=quiz_page)  # 퀴즈 버튼 클릭 시 quiz_page() 함수 호출
button2.grid(row=1, column=1, sticky="nsew")  # 두 번째 버튼을 두 번째 행(row)의 두 번째 열(column)에 배치
...
loading_window.after(5000, show_main_window)  # 5초 뒤에 메인 창 표시
