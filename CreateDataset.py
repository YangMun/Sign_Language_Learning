...
actions = ['me', 'you', 'have', 'nothing', 'home', 'mountain', 'promise', 'name']
seq_length = 30
secs_for_action = 30
...
os.makedirs('dataset', exist_ok=True)  # 데이터 저장 폴더 생성 이미 폴더가 존재하는 경우에도 오류를 발생시키지 않고 넘어감 이 코드에서는 데이터를 저장하기 위한 폴더를 생성하는데 사용
...
while cap.isOpened(): # 비디오 스트림이 열려 있는 동안 반복
    for idx, action in enumerate(actions):
        ...
        np.save(os.path.join('dataset', f'seq_{action}_{created_time}'), full_seq_data) # 시퀀스 데이터를 파일로 저장
