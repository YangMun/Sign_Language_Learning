...
actions = ['next', 'see']  # 액션 목록을 정의합니다.

data = []
for action in actions:
    file_pattern = f'C:/Users/user/Project_CapStone/sentenceDataset/seq_{action}_*.npy'  # 데이터 파일의 경로 패턴을 정의합니다.
...
history = model.fit(
    ...
    epochs=40,
    callbacks=[
        ModelCheckpoint('models/sentence_model.h5', monitor='val_acc', verbose=1, save_best_only=True, mode='auto'),  # 검증 정확도가 가장 높을 때 모델을 저장합니다.
